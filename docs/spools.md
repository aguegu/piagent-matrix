...
`rename()` in so a partial file is never read, files are left alone until they
have been untouched for a second in case a writer ignored that, files are
claimed by hard link
