...
* `.whoami` reports who the bot is — its main room, the admin it was adopted for, why that room was chosen, and its working directory — read out of `main-room.json` rather than told to it. It does not report the model: `agent.json` holds the choice that was recorded, not necessarily what is loaded, and `.info` answers that from the running process. It is told not to read anything else in `DATA_DIR`, where `token.json`, `auth.json` and `crypto/` also sit
