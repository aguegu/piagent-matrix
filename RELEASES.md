...
* **Optional sections of `AGENTS.md`, available and enabled separately.** Everything in `$DATA_DIR/parts/` and `agent/parts/` is *available* — the first is on the data volume, so an operator running a published image can add sections or replace one of ours by name, with no checkout and no rebuild. `AGENT_PARTS` says which are *enabled*, and in what order; the image defaults it to the two a container needs, and a host enables none. That is the only switch governing what the agent is told: adding a section adds a filename to a list rather than another environment variable
