...
* **The image carries what the agent actually uses**, chosen by counting its bash calls rather than by taste: `curl` 2162, `jq` 1223, `python3` 1044 — the three most-used commands, all missing from the first build — plus `procps` for `free` and `ps`. `wget` is the deliberate exception — zero uses against curl's 2174, included because a container is a sandbox: a tool it reaches for and does not find costs a turn, and one more fetcher inside the boundary costs nothing. The same argument does not hold for a host. The absence was quiet, because the agent adapts: asked to schedule something it tried `at`, then `crontab`, then settled for `nohup bash -c 'sleep 300 && …'` and reported success, with three missing tools and no error reaching anyone. `at` and `cron` stay out on purpose — both accept work and silently never run it without their daemon, which is the failure this project has spent two releases removing. 198.8MB to 212.1MB
