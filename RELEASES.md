...
* **Optional sections of `AGENTS.md`, enabled the way nginx enables a site.** `$DATA_DIR/parts/` is *available* — the shipped sections are published there on every start, so an operator running a published image can read what they are turning on, edit one (the managed marker means an edited file is left alone afterwards), or add their own. `$DATA_DIR/parts-enabled/` is *enabled*: a relative symlink turns a section on, the `10-`/`20-` prefixes give the order, `rm` turns it off. Relative because the same link has to resolve from the host and from inside the container. `AGENT_PARTS` seeds a fresh install once and is never consulted again — an empty directory afterwards means everything is off, which is a choice rather than a fault to correct on the next boot
