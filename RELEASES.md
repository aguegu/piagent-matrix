...
* **Scheduling, as a sidecar.** `supercronic` (pinned, checksum verified) reads a crontab file from the data volume and reloads it on change — including when the file is replaced atomically, which was the case worth testing, since a stale inode watch would have made the agent's own edits silently invisible. It runs as its own container rather than a second process beside the bot: a job's whole output is a file in `/data/inbox`, which is the interface the spool already was, so neither needs anything from the other. The scheduler gets the data volume and the workspace, and deliberately not the bot's `env_file`
* **The agent is told where it lives.** A `living-in-container` part, included when `AGENT_SANDBOX=container`: that almost nothing it writes survives, and which three paths do; that `/tmp` is not shared with the container its scheduled jobs run in, so a file left there is invisible to both; and that the host is out of reach — with the awkward detail that `free` and `df /` report the host's figures anyway, while the process list does not, so a number from in here can be real or meaningless depending which one it is. Written from what the containerised bot actually reached for: `/tmp` three times, `free -h` eleven
* **Optional sections of `AGENTS.md`, enabled the way nginx enables a site.** `$DATA_DIR/parts/` is *available* — the shipped sections are published there on every start, so an operator running a published image can read what they are turning on, edit one (the managed marker means an edited file is left alone afterwards), or add their own. `$DATA_DIR/parts-enabled/` is *enabled*: a copy turns a section on, the `10-`/`20-` prefixes give the order, `rm` turns it off, and editing the copy is how you change what a bot is told — with `parts/` keeping the shipped text to compare against. Copies rather than links because the directory is read from the host and from inside the container, and a copy is a file either way. `AGENT_PARTS` seeds a fresh install once and is never consulted again — an empty directory afterwards means everything is off, which is a choice rather than a fault to correct on the next boot
