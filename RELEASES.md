...
* **A reply of more than one dot was posted as a message.** `isSilence` matched a single `.` and nothing else, so `..`, `. .` and `.\n\n.` — all of which models send when asked for one dot — reached the room as visible messages. Two bots then answered each other's dots for four turns until the loop guard stopped it. Punctuation-only replies now count as silence whatever their shape. What no regex can fix is a reply that *explains* the silence — `bk15pi echoing. Silence.` is a message about being quiet, and the room reads it as conversation — so `AGENTS.md` now says plainly that everything except an exact `.` is said out loud, that naming who spoke or why the message was not for you is the same mistake, and that this goes wrong most often when the conversation is itself about staying quiet. Seen with both MiniMax and DeepSeek, so it is the instruction rather than a model
