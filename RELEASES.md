...
* `.whoami` reports who the bot is — what it is and how it is reached, its main room, the admin it was adopted for, why that room was chosen, and its working directory — reading `main-room.json` rather than being told. The template also describes the situation the agent is in: a chat client rather than a terminal, a session per room, one run at a time, answers posted whole and never edited, and the commands the bot handles before the agent sees them. A slash command skips the room briefing, so it would otherwise arrive with no context at all. It does not report the model: `agent.json` holds the choice that was recorded, not necessarily what is loaded, and `.info` answers that from the running process. It is told not to read anything else in `DATA_DIR`, where `token.json`, `auth.json` and `crypto/` also sit
* Prompt templates that describe the bot now live in `prompts/` at the repo root and are installed into `PI_AGENT_DIR/prompts` on every start, with `{{DATA_DIR}}`, `{{BOT_CWD}}` and `{{OUTBOX_DIR}}` substituted — the agent runs in none of those places, so a template naming a path needs an absolute one, and it differs per host. Only files the repo ships are written, so a deployment's own templates are untouched
